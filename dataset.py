...
class AVSEDataset(Dataset):
    ...
    def get_data(self, clean_file, noise_file, noisy_file, mp4_file):
        ...
        if self.clipped_batch:
            if clean.shape[0] > 44800:
                clip_idx = random.randint(0, clean.shape[0] - 44800)
                video_idx = int((clip_idx /560))
                clean = clean[clip_idx:clip_idx + 44800]
                noisy = noisy[clip_idx:clip_idx + 44800]
            else:
                video_idx = -1
                clean = np.pad(clean, pad_width=[0, 44800 - clean.shape[0]], mode="constant")
                noisy = np.pad(noisy, pad_width=[0, 44800 - noisy.shape[0]], mode="constant")
            if len(vr) < 80:
                ...
                max_idx = min(video_idx + 80, len(vr))
            ...
            if len(bg_frames) < 80:
                bg_frames = np.concatenate(
                    (bg_frames, np.zeros((80 - len(bg_frames), img_height, img_width)).astype(bg_frames.dtype)),
                    axis=0)
        else:
            frames = vr.get_batch(list(range(len(vr)))).asnumpy()
            # print("f1 ", frames.shape)
            frames = self.resize_video_frames(frames)
            ...
            bg_frames = np.array(
                [cv2.cvtColor(frames[i], cv2.COLOR_RGB2GRAY) for i in range(len(frames))]).astype(np.float32)
            bg_frames /= 255.0
        # print("f2 ",bg_frames[np.newaxis, ...].shape)
        # print("shape ",bg_frames[np.newaxis,...].shape)
        return torch.tensor(noisy), torch.tensor(clean), torch.tensor(bg_frames[np.newaxis, ...])
...
class AVSEDataModule(LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        # print(len(self.dev_dataset_batch))
